...
Fact_Quotation_Item is added at quotation coverage item grain because the source system contains quotation item records. This preserves lower-level details such as coverage type, coverage amount, and deductible amount. In the current ERD, it is kept lean by connecting directly to Dim_Coverage, Dim_Vehicle, and Dim_Quotation. It conceptually inherits context from the quotation header without physically repeating all header-level dimensions directly on the item fact.
...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Fact_Quotation**      | Dim_Date, Dim_Customer, Dim_Provider, Dim_Vehicle, Dim_Agent, Dim_Package, Dim_Quotation_Status, Dim_Quotation                                            | These dimensions describe the quotation header context: when the quote was created, who requested it, which vehicle is quoted, which provider/agent/package it belongs to, what quotation status it has, and which quotation identifier it represents.                                                                       |
| **Fact_Quotation_Item** | **Direct:** Dim_Coverage, Dim_Vehicle, Dim_Quotation<br>**Inherited:** Dim_Date, Dim_Customer, Dim_Agent, Dim_Provider, Dim_Package, Dim_Quotation_Status | This fact focuses on coverage-item context. Dim_Coverage describes the coverage type, Dim_Vehicle specifies the exact vehicle for the item line, while Dim_Quotation links each item back to the quotation header. It logically inherits header-level dimensions without requiring direct physical relationships in the ERD. |
| **Fact_Policy**         | Dim_Date, Dim_Customer, Dim_Provider, Dim_Vehicle, Dim_Agent, Dim_Package, Dim_Quotation, Dim_Policy, Dim_Policy_Status                                   | These dimensions support policy-level analysis and allow policies to be analyzed by date, customer, provider, vehicle, agent, package, originating quote, policy status, and policy identifier.                                                                                                                              |
| **Fact_Payment**        | **Direct:** Dim_Date, Dim_Policy, Dim_Payment_Status, Dim_Payment_Method<br>**Resolved:** Dim_Customer, Dim_Provider, Dim_Vehicle                         | Direct dimensions support payment operation analysis by execution date, status, method, and linked policy. Contextual dimensions like Customer, Provider, and Vehicle are resolved transitively through the associated policy.                                                                                               |
| **Fact_Cancellation**   | **Direct:** Dim_Date, Dim_Cancellation_Reason, Dim_Policy<br>**Resolved:** Dim_Customer, Dim_Provider, Dim_Vehicle                                        | Direct dimensions support cancellation analysis by execution date, specific cancellation reason, and linked policy. Contextual dimensions like Customer, Provider, and Vehicle are resolved transitively through the associated policy. Refund amount remains a measure here.                                                |

---

## ERD Adjustment Rules

The ERD should keep Fact_Quotation and Fact_Quotation_Item as separate fact tables. This is not duplication because the two facts have different grains: Fact_Quotation is quote header level, while Fact_Quotation_Item is coverage item level. Dim_Coverage should connect only to Fact_Quotation_Item, because coverage context belongs to the item grain. Both fact tables establish a connection to Dim_Vehicle as a direct relationship. Fact_Quotation_Item also connects to Dim_Quotation so item rows can be grouped back to the quotation header, allowing inherited access to Date, Customer, Agent, Provider, Package, and Quotation Status.
...
Fact tables should not be directly connected to each other as the main semantic model design. Fact_Quotation and Fact_Quotation_Item share Dim_Quotation and Dim_Vehicle, while Fact_Policy, Fact_Payment, and Fact_Cancellation share Dim_Policy. This allows related business processes to be analyzed together without using direct fact-to-fact filtering. All normal dimension-to-fact relationships should be one-to-many, with the dimension on the one side and the fact on the many side.
...
The model now keeps five fact tables: Fact_Quotation, Fact_Quotation_Item, Fact_Policy, Fact_Payment, and Fact_Cancellation. Fact_Quotation represents the sales quote header and owns quote-level premium/conversion analysis. Fact_Quotation_Item represents the coverage details inside each quotation and utilizes explicit inheritance rules for header context. Fact_Policy represents the accepted business contract and policy-level record, directly linking back to the originating quote. Fact_Payment represents the financial transaction after the policy exists, utilizing resolved connections for customer and vehicle data. Fact_Cancellation represents the cancellation event after the policy exists and supports cancellation reason and refund analysis. The updated ERD properly accounts for direct, inherited, and resolved dimension assignments to avoid redundant modeling structures.