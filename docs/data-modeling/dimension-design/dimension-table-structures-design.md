...
The current source data only provides `cancellation.cancellation_reason`.  
Therefore, this dimension will be designed based on distinct cancellation reason values from the source.

Attributes such as `reason_group` and `is_customer_initiated` are not included in the current scope because they cannot be directly populated from the source data. These attributes can be added later if the business provides a mapping table or if explicit ETL derivation rules are confirmed.

| Column                    | Type Suggestion | Description                                                               |
| ------------------------- | --------------- | ------------------------------------------------------------------------- |
| `cancellation_reason_key` | BIGINT          | Surrogate key.                                                            |
| `cancellation_reason`     | STRING          | Cancellation reason value from source `cancellation.cancellation_reason`. |
| `created_at`              | TIMESTAMP       | Gold row creation time.                                                   |
| `updated_at`              | TIMESTAMP       | Gold row update time.                                                     |
