...
| M-18 | Written Premium by Month | Trend | Line | `CALCULATE([Total Written Premium (VND)], DATESMTD(dim_date[full_date]))` | Monthly written premium trend. | Track premium production trends over time and identify changes in business volume or product mix. | M-05, dim_date | | Draft |
...
| KPI ID | Measure Name | Group | Chart Type | DAX Formula | Business Meaning | Why It Is Important? | Database Source Table | Calculation Column | Status |
|--------|-------------|-------|------------|-------------|-----------------|---------------------|----------------------|-------------------|--------|
| M-20 | Quotations PY | Time Intel | | `CALCULATE([Total Quotations], SAMEPERIODLASTYEAR(dim_date[full_date]))` | Quotation volume in previous year | Baseline for calculating year-on-year growth. Avoid confusing it with the 365-day period. | fact_quotation, dim_date | quotation_date_key | Draft |
...
| N2-04 | Quotations PY | Time Intel | | `CALCULATE([Total Quotations], SAMEPERIODLASTYEAR(dim_date[full_date]))` | Quotation volume in previous year | Baseline for calculating year-on-year growth. Avoid confusing it with the 365-day period. | fact_quotation, dim_date | quotation_date_key | Draft |
...
| N3-05 | Quotation Age (day) | Agent | Column | `AVERAGEX(FILTER(fact_quotation, RELATED(dim_quotation_status[quotation_status_code]) = "QUOTED"), DATEDIFF(RELATED(dim_date[full_date]), TODAY(), DAY))` | Average age of open quotations based on the quotation date. (Calculated using the quotation date role: fact_quotation[quotation_date_key] → dim_date[date_key].) | Identifies aging quotations that may require follow-up before expiring or being lost. | fact_quotation, dim_date | quotation_date_key | Draft |
